...
import { Code, PlayCircle, ArrowLeft, Clock, Save, ExternalLink, CheckCircle } from "lucide-react";
import { motion } from "framer-motion";
import createJudge0Client, { SubmissionResult } from "@/lib/judge0";
...
const CodeQuestion = () => {
  ...
  useEffect(() => {
    if (question) {
      setCode(question.startingCode[selectedLanguage as keyof typeof question.startingCode] || "");
    }
  ...
  const runCode = async () => {
    ...
    try {
      ...
      const testCaseInputs = question.testCases.map(tc => {
    ...
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-slate-950">
      ...
      <header className="px-6 py-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h1 className="font-bold text-xl text-gray-900 dark:text-white">{question.title}</h1>
        ...
        <div className="flex items-center">
          <div className="mr-4 flex items-center">
            ...
            <span className={`font-mono ${timeLeft < 60 ? "text-red-500" : "text-gray-600 dark:text-gray-300"}`}>
              {formatTime(timeLeft)}
            </span>
          </div>
          ...
          <Panel defaultSize={35} minSize={20}>
            <div className="h-full overflow-auto p-6 bg-white dark:bg-slate-900">
              <div className="prose dark:prose-invert max-w-none">
                <div dangerouslySetInnerHTML={{ __html: question.fullDescription.replace(/\n/g, "<br />") }} />
                
                <h3 className="mt-6">Example Test Cases</h3>
                <div className="mt-4 space-y-4">
                  {question.testCases.map((testCase, index) => (
                    <div key={index} className="bg-gray-50 dark:bg-slate-800 p-4 rounded-md">
      ...
      >
        <p className="text-gray-600 dark:text-gray-400">
          Your time has expired. You can still submit your solution, but it will be marked as late.
        </p>
      </Modal>
